...
class TestInstanceSettings(ClickhouseTestMixin, APILicensedTest):
    ...
    def test_update_recordings_ttl_setting(self):
        response = self.client.get("/api/instance_settings/RECORDINGS_TTL_WEEKS")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["value"], 3)

        response = self.client.patch(
            "/api/instance_settings/RECORDINGS_TTL_WEEKS", {"value": 5}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_recordings_performance_events_ttl_setting(self):
        response = self.client.get(
            "/api/instance_settings/RECORDINGS_PERFORMANCE_EVENTS_TTL_WEEKS"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()["value"], 3)
