...
class FeatureFlagSerializer(TaggedItemSerializerMixin, serializers.HyperlinkedModelSerializer):
    ...
    def validate_filters(self, filters):
        ...
        if filters.get("multivariate"):
            if any(key not in variants for key in payloads):
    ...
    def create(self, validated_data: Dict, *args: Any, **kwargs: Any) -> FeatureFlag:
        ...
        variant_rollout_sum = sum(
            variant.get("rollout_percentage") for variant in variants
        )
    ...
    def update(self, instance: FeatureFlag, validated_data: Dict, *args: Any, **kwargs: Any) -> FeatureFlag:
        request = self.context["request"]
        if validated_key := validated_data.get("key", None):
    ...
    def _update_filters(self, validated_data):
        ...
        if active := validated_data.get("active", None):
...
def _create_usage_dashboard(feature_flag: FeatureFlag, user):
    ...
    usage_dashboard = Dashboard.objects.create(
        name=f"Generated Dashboard: {feature_flag.key} Usage",
        description=f"This dashboard was generated by the feature flag with key ({feature_flag.key})",
...
class FeatureFlagViewSet(TaggedItemViewSetMixin, StructuredViewSetMixin, ForbidDestroyModel, viewsets.ModelViewSet):
    ...
    def dashboard(self, request: request.Request, **kwargs):
        ...
        try:
            ...
            return Response(
                {"success": False, "error": "Unable to generate usage dashboard"},
                status=400,
            )
        ...
        flags.extend(
            {
                "feature_flag": FeatureFlagSerializer(
                    feature_flag, context=self.get_serializer_context()
                ).data,
