...
class TestInstanceSettings(APIBaseTest):
    ...
    def test_list_instance_settings(self):
        ...
        response = self.client.get("/api/instance_settings/")
    ...
    def test_retrieve_secret_setting(self):
        ...
        response = self.client.get("/api/instance_settings/EMAIL_HOST_PASSWORD")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        json_response = response.json()
    ...
    def test_update_setting(self):
        ...
        response = self.client.patch(
            "/api/instance_settings/AUTO_START_ASYNC_MIGRATIONS", {"value": True}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_updating_email_settings(self):
        ...
        with self.settings(SITE_URL="http://localhost:8000", CELERY_TASK_ALWAYS_EAGER=True):
            response = self.client.patch(
                "/api/instance_settings/EMAIL_DEFAULT_FROM",
    ...
    def test_update_integer_setting(self):
        response = self.client.patch(
            "/api/instance_settings/ASYNC_MIGRATIONS_ROLLBACK_TIMEOUT",
            {"value": 48343943943},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_cant_update_setting_that_is_not_overridable(self):
        response = self.client.patch(
            "/api/instance_settings/MATERIALIZED_COLUMNS_ENABLED", {"value": False}
        )
